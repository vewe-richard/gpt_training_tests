...
class Net(nn.Module):
    ...
    def forward(self, x):
        ...
        # Pass the tensor through the third fully connected layer (output layer)
        x = self.fc2(x)
...
for i in range(10):
    ...
    try:
        ...
        # Print the filename and predicted label on the same line
        print(f"{name}: Predicted digit: {predicted.item()}")
